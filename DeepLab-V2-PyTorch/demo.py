...
# -i
# /home/lianghuizhu/data/VOC2012/VOCdevkit/VOC2012/JPEGImages/2007_000452.jpg
...
@click.option("--crf", is_flag=True, show_default=True, help="CRF post-processing")
@click.option("--video_path", type=click.Path(exists=True), required=True, show_default=True, help="Video path info")
def video(config_path, model_path, cuda, crf, video_path):
    """
    Inference from camera stream
    """
# video -c configs/voc12.yaml -m data/models/Deeplabv2_pseudo_segmentation_labels_2gpu/deeplabv2_resnet101_msc/train_aug_cls/checkpoint_final.pth --video_path test3.MOV
    # Setup
    CONFIG = Dict(yaml.load(config_path))
    device = get_device(cuda)
    torch.set_grad_enabled(False)
    ...
    output_path = '/home/lianghuizhu/Project8_WSSS/reference/DRS/DeepLab-V2-PyTorch/test_output/' + video_path.split('.')[0] + ".mp4"
    if True:
        ...
        size = predict_size(np.ones((im_width, im_height)))
        print(size)
        # write_cap = cv2.VideoWriter(output_path, fourcc, 25, (237, 513))
        write_cap = cv2.VideoWriter(output_path, fourcc, 25, size) # 1560 720
    ...
    while cap.grab():
        ...
        # colormap = colormap.transpose(2, 1, 0)
        time_str = ''
        if True:
            write_cap.write((colormap * 0.3 + raw_image * 0.7).astype(np.uint8))
